...
class CanvasView(Gtk.Box):
    ...
    def on_stage_select_drag_update(self, _gesture: Gtk.GestureDrag, offset_x: float, offset_y: float):
        ...
        if self.port_drag_active and self.link_preview_source_id:
            ...
            target = self.active_drag_target(source_id or "", x, y)
            if target:
    ...
    def start_node_drag(
        ...
        *,
        ...
        if not node:
            return
        if previous_selection_set is None:
            previous_selection_set = set(self.selected_node_ids)
        ...
        if node_id not in self.drag_group_origins:
            self.drag_group_origins[node_id] = (node.x, node.y)
        self.drag_history_captured = False
        self.drag_guide_x = None
        self.drag_guide_y = None
        self.suppress_stage_click_once = True
    ...
    def on_node_drag_update(
        ...
        if observed_stage:
            ...
            # Keep offset-based drag motion as primary source and only trust observed
            # coordinates when they are reasonably close. This avoids large jumps on
            # GTK stacks where translated coordinates can drift while node widgets
            # move under the pointer during drag updates.
            drift = abs(observed_x - pointer_stage_x) + abs(observed_y - pointer_stage_y)
            tolerance = max(24.0, float(self.card_screen_width()) * 0.6)
            if drift <= tolerance:
